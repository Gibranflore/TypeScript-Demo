...
export const InstagromApp = () => {
  ...
  const [optimisticComments, addOptimisticComments] = useOptimistic(comments, (currentsComments, newCommentsText: string) => {
    lastId++
    return [
      ...currentsComments,
      {
  ...
  const handleAddComment = async (FormData: FormData) => {
    const setMessage = FormData.get('post-message') as string

    addOptimisticComments(setMessage)
    ...
    startTransition(async() => {
      ...
      await new Promise((resolve) => setTimeout(resolve, 3000))
        ...
        toast('Error al publicar comentario',{
